...
In this repository, the `AGENTS.md` file lives at the repo root. Update that file to include the `ExecPlans` section described below so Codex will know when to use `.agent/PLANS.md`.
...
When writing complex features or significant refactors, use an ExecPlan (as described in .agent/PLANS.md) from design to implementation. Store new ExecPlans under .agent/Tasks/ (prefer .agent/Tasks/<task-slug>/exec-plan.md).
```
