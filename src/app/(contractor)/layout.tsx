...
export default async function ContractorLayout({
  ...
  const contractor = contractorData as { id: string; business_name: string | null; email: string } | null;
  ...
  const initials = contractor?.business_name
    ? contractor.business_name.split(' ').map(w => w[0]).join('').toUpperCase().slice(0, 2)
                ...
                <div className="flex items-center justify-start gap-2 p-2">
                  <div className="flex flex-col space-y-0.5">
                    <p className="text-sm font-medium">
                      {contractor?.business_name ?? 'Your Business'}
                    </p>
