...
```bash
cd .codex/.home/skills/dev-browser
./server.sh &
```
...
> **CRITICAL: Always run scripts from `.codex/.home/skills/dev-browser/`**
>
> Scripts must be executed from the `.codex/.home/skills/dev-browser/` directory. The `@/` import alias (e.g., `@/client.js`) is configured in this directory's `tsconfig.json` and `package.json`. Running from any other directory will fail with:
>
> ```
...
```bash
cd .codex/.home/skills/dev-browser && npx tsx <<'EOF'
