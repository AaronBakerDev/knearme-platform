...
export const toolDefinitions = [
  {
    ...
    description:
      'Create a new draft case-study project. Use this when the contractor wants to start a new project. Returns the project ID and lists any missing fields needed for publishing.',
    inputSchema: {
      type: 'object',
      properties: {
  ...
  },
  {
    ...
    description:
      'Add images to a project draft. Use this when the contractor uploads photos. Accepts file IDs from ChatGPT uploads.',
    inputSchema: {
      type: 'object',
      properties: {
  ...
  },
  {
    ...
    description:
      'Update the narrative sections of a project: summary, challenge, solution, results, and outcome highlights. The description is auto-composed from these sections.',
    inputSchema: {
      type: 'object',
      properties: {
        project_id: { type: 'string', description: 'Project UUID' },
  ...
  },
  {
    ...
    description:
      'Publish the project, making it visible on the public portfolio. Requires all mandatory fields to be filled. This action is destructive - ask for confirmation before calling.',
    inputSchema: {
      type: 'object',
      properties: {
        project_id: { type: 'string', description: 'Project UUID' },
...
/**
 * Widget template types for ChatGPT rendering.
 * Each template corresponds to a specific UI in the widget bundle.
 *
 * @see /mcp-server/widgets/src/templates/
 */
type WidgetTemplate = 'project-draft' | 'project-media' | 'project-status' | 'project-list';
 ...
 */
function buildWidgetMeta<T extends Record<string, unknown>>(
...
export async function handleReorderProjectMedia(
  ...
  const images = imagesResult.data.images.map((img) =>
    toImageOutput(img as unknown as Record<string, unknown>)
  );
...
export async function handleListContractorProjects(
  ...
  const projects = result.data.projects.map((p) =>
    toProjectOutput(p as unknown as Record<string, unknown>)
  );
