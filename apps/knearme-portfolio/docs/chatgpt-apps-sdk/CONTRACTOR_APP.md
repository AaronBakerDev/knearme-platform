...
8. Widget calls `publish_project` to publish the project.
9. ChatGPT confirms and returns the public URL from `publish_project` plus a deep link to the web app for full management.
...
For full schemas and output shapes, see `MCP_CONTRACTOR_INTERFACE.md`. Summary below reflects the MCP server implementation.

### `create_project_draft`

Inputs: optional `title`, `project_type`, `city`, `state`, `summary`, `challenge`, `solution`, `results`, `outcome_highlights`.

Outputs: `structuredContent` with `project_id` + `missing_fields`; `_meta` includes `project-draft` widget data and the draft project.

### `add_project_media`

Inputs: `project_id` plus either `files[]` (ChatGPT `file_id`, `filename`, `content_type`, optional `image_type`, `alt_text`, `display_order`, `width`, `height`) or `images[]` (URL fallback with optional `filename`, `image_type`, `alt_text`).

Outputs: `structuredContent` with `media_count`, `missing_fields`, and `upload_status` (plus optional `upload_errors`). `_meta` includes `project-media` widget data and `uploads` for signed URLs.

### `reorder_project_media`

Inputs: `project_id`, ordered `image_ids`.

### `set_project_hero_media`

Inputs: `project_id`, `hero_image_id`.

### `set_project_media_labels`

Inputs: `project_id`, `labels[]` with `image_id`, optional `image_type`, `alt_text`.

### `update_project_sections`

Inputs: `summary`, `challenge`, `solution`, `results`, `outcome_highlights`.

Outputs: `structuredContent` with `missing_fields` + `can_publish`; `_meta` includes `project-draft` widget data.

### `update_project_meta`

Inputs: `title`, `project_type`, `city`, `state`, `duration`, `tags`, `materials`, `techniques`, `seo_title`, `seo_description`.

Outputs: `structuredContent` with `missing_fields` + `can_publish`; `_meta` includes `project-draft` widget data.

### `publish_project`

Inputs: `project_id`. Output includes the public URL. Destructive; require confirmation.

### `unpublish_project`

Inputs: `project_id`. Output returns draft status.

### `get_project_status`

Inputs: `project_id`. Output includes status, missing fields, and publish readiness.

### `list_projects`

Inputs: optional `status`, `limit`, `offset`. Output includes `count`, `has_more`, and project cards for the widget.
...
1. **Draft review widget**
   ...
   - Edit inline fields
   - CTA: Publish project (when `can_publish` is true)
...
- Use `destructiveHint` for `publish_project`.
